...
public class AuditDAOImpl extends AbstractAuditDAOImpl {
	...
	protected int findAuditEntries(
		...
		//TODO
		List<Pair<Long, Long>> kvIds = new ArrayList<>();
		for (Pair<String, Serializable> pair : kvs) {
			Pair<Long, Serializable> searchKeyPair = propertyValueDAO.getPropertyValue(pair.getFirst());
			Pair<Long, Serializable> searchValuePair = propertyValueDAO.getPropertyValue(pair.getSecond());
			
			if (searchKeyPair == null || searchValuePair == null) continue;
			
			kvIds.add(new Pair<>(searchKeyPair.getFirst(), searchValuePair.getFirst()));
		}
